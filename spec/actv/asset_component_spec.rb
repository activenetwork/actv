  ...
  describe "#prices" do
    before(:each) do
      stub_request(:post, "http://api.amp.active.com/v2/assets.json").
        with(:body => {"id"=>""}, :headers => {'Accept'=>'application/json', 'Accept-Encoding'=>'gzip;q=1.0,deflate;q=0.6,identity;q=0.3', 'Content-Type'=>'application/x-www-form-urlencoded'}).
        to_return(body: fixture("valid_component_asset.json"), headers: {})
    end
